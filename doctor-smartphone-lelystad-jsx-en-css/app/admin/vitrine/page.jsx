...
import VitrineAdminPage from "@/components/admin/vitrine-admin-page"
...
export default async function AdminVitrinePage({ searchParams }) {
      ...
      <div className="min-h-screen bg-gray-100 flex items-center justify-center px-4">
        <div className="bg-white border rounded-xl p-8 w-full max-w-md">
          ...
          <p className="text-sm text-gray-600 text-center mb-6">
            Log in om de Vitrine te beheren.
          </p>
          ...
          <form action="/api/admin/login" method="post" className="space-y-4">
            ...
            <input type="hidden" name="redirect" value="/admin/vitrine" />
  ...
  try {
    await connectDB()
    items = await VitrineItem.find().sort({ createdAt: -1 }).lean()
    items = JSON.parse(JSON.stringify(items))
  } catch (e) {
    console.error("Database verbinding mislukt in AdminVitrinePage:", e)
    dbError = true
  }
          ...
          <div className="bg-white border rounded-xl p-6">
            <div className="flex items-center justify-between gap-4">
              <div>
                ...
                <p className="text-gray-600">Vitrine beheer</p>
              </div>
              <a
                href="/api/admin/logout?redirect=/admin/vitrine"
  ...
  return <VitrineAdminPage initialItems={items} />
}
